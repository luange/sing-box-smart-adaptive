...
func (c *outboundCoordinator) MaybeLearnTCP(
	...
	// Perf: gate non-direct BEFORE resolve. Proxy leaves used to dominate
	// addr_mismatch (dest=site, peer=proxy) and hide real learn failures.
	if !verdictIsEmptyDirect(outboundDialer) {
		backend.Skip()
		c.noteSkipReason(verdictSkipNonDirect)
		c.debug("eBPF verdict learn skip: reason=", verdictSkipNonDirect, " non-direct outbound")
		return
	}
