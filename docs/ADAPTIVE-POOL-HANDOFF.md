...
> **面向**：接手的同事与其它 AI  
> **日期**：2026-07-31  
> **当前生产版本**：`1.14.0-beta.3-reF1nd-luan-adaptive.rc31-probe-hygiene`  
...
| 项 | 值 |
|----|-----|
| 产品线 | **reF1nd v1.14.0-beta.3 + Smart + AdaptivePool**（**不用**官方 beta.4，见 §2） |
...
| 源码 worktree（Mac） | `/Users/luan/Documents/Codex/2026-06-18/version-3-9-services-sub-store/work/a51-beta1-adaptive` |
| 本地分支 | `rc18-beta3-adaptive` @ `77eecee77` |
| 推送用 GitHub | https://github.com/luange/sing-box-luan-smart |
| GitHub 分支 | `adaptive/rc31-probe-hygiene`（跟踪本地 `rc18-beta3-adaptive`） |
...
## 2. 版本线决策：为什么是 beta.3 不是 beta.4

### 2.1 结论

- **产品线锁定**：reF1nd **`v1.14.0-beta.3-reF1nd`**（tag `78318d46`）+ Adaptive。  
- **官方**已有 `v1.14.0-beta.4`，**reF1nd 没有 beta.4**。  
- 硬并官方 beta.4 ≈ 三方分叉（SagerNet core + reF1nd provider/reload + Adaptive 钩子），冲突面约 **127 files / changed-in-both**，曾尝试 bulk merge → `DomainMatchStrategy` 等编译断 → **reset 回 beta.3**。  
- 不是“算法不能迁”，而是 **挂载面（group/option/box/clash/dns/route）差得远**，需完整 re-port + soak，不能当小版本 bump。

### 2.2 对比数字（当时实测）

| 对比 | 量级 |
|------|------|
| HEAD vs 官方 beta.4 | 我方独有 ~325 commits / 对方 ~211 |
| merge-tree “changed in both” | ~127 文件 |
| reF1nd 相对官方 beta.3 | ~74 commits（provider/reload/loadbalance 等） |
| Adaptive 专有文件 | 官方侧不存在（如 `adapter/adaptive_pool.go`、clash adaptive API） |

**接手时**：除非明确排期 “port to beta.4 / 等 reF1nd beta.4”，否则 **不要** 把官方 testing 混进生产分支。
...
```text
/Users/luan/Documents/Codex/2026-06-18/version-3-9-services-sub-store/work/a51-beta1-adaptive
```

- 分支：`rc18-beta3-adaptive`  
- HEAD：`77eecee77`  
...
```bash
cd /Users/luan/Documents/Codex/2026-06-18/version-3-9-services-sub-store/work/a51-beta1-adaptive
git remote -v
git status -sb
```
