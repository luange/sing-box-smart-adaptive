...
BACKUP=${BACKUP:-/root/codex-backups/${STAMP}-before-${VERSION_STEM}}
CHANGED=0
ERROR_OFFSET=0
...
rollback() {
  ...
  if [ "$CHANGED" -eq 1 ]; then
    echo "rollout failed; restoring $BACKUP" >&2
    rc-service singbox stop >/dev/null 2>&1 || true
    cp -f "$BACKUP/sing-box" "$TARGET"
    chmod 0755 "$TARGET"
...
if [ -f "$ERROR_LOG" ] && tail -c "+$((ERROR_OFFSET + 1))" "$ERROR_LOG" 2>/dev/null \
  ...
  | grep -q .; then
  echo 'panic/fatal found in recent sing-box stderr' >&2
  exit 1
fi
