...
version="${1:-}"
if [[ -z "$version" ]]; then
	echo "version is required (expected SemVer, optionally with +smart.N.N build metadata)" >&2
  exit 2
fi
...
if [[ ! "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-(alpha|beta|rc)\.[0-9]+)?(\+smart\.[0-9]+\.[0-9]+)?$ ]]; then
	echo "invalid version '$version'; use SemVer or SemVer+smart.N.N" >&2
  exit 2
fi
